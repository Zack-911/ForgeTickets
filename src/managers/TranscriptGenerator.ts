...
export class TranscriptGenerator {
    ...
    public static async build(
        ticket: Ticket,
        channel: TextChannel,
        format: TranscriptFormat = TranscriptFormat.HTML
    ): Promise<{ html: string | null; text: string | null }> {
        const messages = await this._fetchAllMessages(channel)
        const sorted = [...messages.values()].sort((a, b) => a.createdTimestamp - b.createdTimestamp)

        const html =
            format === TranscriptFormat.HTML || format === TranscriptFormat.Both
                ? this._buildHTML(ticket, sorted, { name: "" } as TicketCategory)
                : null

        const text =
            format === TranscriptFormat.Text || format === TranscriptFormat.Both
