...
import {
    Ticket, MongoTicket,
    TicketCategory, MongoTicketCategory,
    TicketTeam, MongoTicketTeam,
    BlacklistEntry, MongoBlacklistEntry,
    TicketPanel, MongoTicketPanel,
    GuildSettings, MongoGuildSettings,
    TicketState,
} from "./entities"
...
export class TicketsDatabase extends DataBaseManager {
    ...
    public entityManager = {
        ...
        mongodb:  [MongoTicket, MongoTicketCategory, MongoTicketTeam, MongoBlacklistEntry, MongoTicketPanel, MongoGuildSettings],
    ...
    public static async getActiveTickets(guildID: Snowflake): Promise<Ticket[]> {
        const all = await this.db.getRepository(this.entities.Ticket).find({ where: { guildID, deleted: false } as any }) as Ticket[]
        return all.filter(t => t.isActive())
    }
    ...
    public static async getCategoriesByGuild(guildID: Snowflake): Promise<TicketCategory[]> {
        return await this.db.getRepository(this.entities.Category).find({ where: { guildID } as any }) as TicketCategory[]
    }
    ...
    public static async getBlacklist(guildID: Snowflake): Promise<BlacklistEntry[]> {
        const entries = await this.db.getRepository(this.entities.Blacklist).find({ where: { guildID } as any }) as BlacklistEntry[]
    ...
    public static async isBlacklisted(guildID: Snowflake, userID: Snowflake, roleIDs: Snowflake[]): Promise<BlacklistEntry | null> {
        const active = await this.getBlacklist(guildID)
        return active.find(e =>
            (e.type === "user" && e.targetID === userID) ||
            (e.type === "role" && roleIDs.includes(e.targetID))
        ) ?? null
    }
    ...
    public static async getSettings(guildID: Snowflake): Promise<GuildSettings> {
        const existing = await this.db.getRepository(this.entities.Settings).findOneBy({ guildID } as any) as GuildSettings | null
